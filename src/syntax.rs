...
pub enum Exp<Ann> {
  ...
  // First argument should always be a variable
  ...
  Match {
    expr: Box<Exp<Ann>>,
    default: Box<Exp<Ann>>,
    arms: Vec<(SnakeType, Vec<String>, Exp<Ann>)>,
    ann: Ann,
  },
  // intermediate forms
  MakeTypeInstance {
...
impl<Ann> Exp<Ann> {
    ...
    Ann: Clone,
  {
    match self {
      ...
      | Exp::TypeDefs { ann: a, .. }
      | Exp::Match { ann: a, .. }
      | Exp::MakeTypeInstance { ann: a, .. }
      | Exp::MatchType { ann: a, .. }
      | Exp::GetTypeFields(_, a) => a.clone(),
    }
  }
    ...
    F: FnMut(&Ann) -> Ann2,
  {
    match self {
      ...
      Exp::TypeDefs { decls, body, ann } => Exp::TypeDefs {
        decls: decls.clone(),
        body: Box::new(body.map_ann(f)),
        ann: f(ann),
      },
      Exp::Match {
        ...
        expr: Box::new(expr.map_ann(f)),
        default: Box::new(default.map_ann(f)),
        arms: arms
          .iter()
          .map(|(match_case, params, case)| (match_case.clone(), params.clone(), case.map_ann(f)))
          .collect(),
        ann: f(ann),
      },
      Exp::MakeTypeInstance {
        ...
        typetag: *typetag,
        fields: Box::new(fields.map_ann(f)),
        ann: f(ann),
      },
      Exp::MatchType { expr, typetag, ann } => Exp::MatchType {
        expr: Box::new(expr.map_ann(f)),
        typetag: *typetag,
        ann: f(ann),
      },
      Exp::GetTypeFields(custom_type, ann) => {
        Exp::GetTypeFields(Box::new(custom_type.map_ann(f)), f(ann))
      }
    }
  }
}
...
impl<Ann> SeqExp<Ann> {
    ...
    Ann: Clone,
  {
    match self {
      ...
      | SeqExp::Let { ann: a, .. }
      | SeqExp::If { ann: a, .. }
      | SeqExp::MakeTypeInstance { ann: a, .. }
      | SeqExp::MatchType { ann: a, .. }
      | SeqExp::GetTypeFields(_, a) => a.clone(),
    }
  }
    ...
    F: FnMut(&Ann) -> Ann2,
  {
    match self {
      ...
      SeqExp::Prim2(op, imm1, imm2, a) => SeqExp::Prim2(*op, imm1.clone(), imm2.clone(), f(a)),
      SeqExp::ArraySet {
        array,
        index,
        new_value,
      ...
      SeqExp::MatchType { expr, typetag, ann } => SeqExp::MatchType {
        expr: expr.clone(),
        typetag: *typetag,
        ann: f(ann),
      },
      SeqExp::GetTypeFields(type_tag, ann) => SeqExp::GetTypeFields(type_tag.clone(), f(ann)),
    }
  }
}
